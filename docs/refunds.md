# Refund service reference application

[Back to the framework overview](../README.md). The original refund-specific CLI remains available.
...
The default container serves the two-application core framework report. Use `agent-reliability serve-demo` to serve the refund-only report. It exposes no live
model execution endpoint and needs no API keys. See the [Cloud Run deployment recipe](cloud-run.md).
